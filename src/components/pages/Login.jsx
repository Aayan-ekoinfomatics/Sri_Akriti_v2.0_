...
const Login = () => {
  ...
  return (
    <div className='flex justify-center items-center px-5 w-full h-[70vh]'>
      ...
      <form onSubmit={formSubmit} className='md:px-8 flex-1 flex flex-col items-center justify-evenly md:mx-5 md:max-w-[450px] md:max-h-[450px] py-2 bg-[#E3E3E3] lg:py-5'>
        <h1 className='lora py-4 text-[30px] font-[500] tracking-[1px] italic' >Login</h1>
        <div className='flex flex-col justify-evenly gap-4 w-[70%]'>
          ...
          <div className='w-full flex justify-center items-center gap bg-white pr-1'>
            ...
            <span className='w-full max-w-[10%] pl-[6px] flex justify-center items-center'>{passwordView ? <VisibilityOffIcon className='text-gray-600' fontSize='small' onClick={() => setPasswordView(!passwordView)} /> : <VisibilityIcon className='text-gray-600' fontSize='small' onClick={() => setPasswordView(!passwordView)} />}</span>
          </div>
          {
            <p className='text-[12px] text-red-500'>{errorText}</p>
          }
          <button className='bg-black text-white p-3 px-12 text-[14px] poppins transition-all duration-300 active:scale-[0.95] tracking-[4px] font-[300]'>LOGIN</button>
        </div>
        <div className='w-full flex justify-center items-center mt-8'>
          <Link to='/signup'><h1 className='text-[11px] border-b pb-2'>New User ? Sign Up</h1></Link>
        </div>
      </form>
    </div>
  )
